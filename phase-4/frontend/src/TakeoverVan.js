...
function TakeoverVan() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formDataToSubmit.ip_username || !formDataToSubmit.ip_id || isNaN(formDataToSubmit.ip_tag)) {
      alert('All fields are required and Tag must be a valid number.');
      return;
    }

    try {
      const response = await axios.post(
        'http://127.0.0.1:5000/api/procedure/takeover_van',
        formDataToSubmit,
        { headers: { 'Content-Type': 'application/json' } }
      );
      if (response.data.success) {
        alert('Van control successfully transferred!');
        setFormData({
          ip_username: '',
          ip_id: '',
          ip_tag: ''
        ...
        alert('Error transferring van control');
      }
    } catch (error) {
